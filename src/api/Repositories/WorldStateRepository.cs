...
public class WorldStateRepository : IWorldStateRepository
{
    ...
    public WorldStateRepository(IBlobArtifactRepository blobRepository, IUserContextService userContextService)
    {
    ...
    public async Task<string> SaveNewVersionAsync(WorldState worldState, string? priorVersionId = null, string? sourceRequestId = null, string? environment = null, bool llmAssisted = false)
    {
        ...
        var blobPath = ArtifactPathHelper.GetWorldStateVersionPath(userId, versionId);
        ...
        var currentPath = ArtifactPathHelper.GetWorldStateCurrentPath(userId);
        await _blobRepository.SaveBlobAsync(currentPath, currentVersionContent);

        return versionId;
    }

    public async Task<WorldState?> GetCurrentVersionAsync(string userId)
    {
        var currentPath = ArtifactPathHelper.GetWorldStateCurrentPath(userId);
    ...
    public async Task<WorldState?> GetVersionAsync(string userId, string versionId)
    {
        var blobPath = ArtifactPathHelper.GetWorldStateVersionPath(userId, versionId);
        ...
        var versionedArtifact = JsonSerializer.Deserialize<VersionedWorldState>(jsonContent, _jsonOptions);
        return versionedArtifact?.WorldState;
    }

    public async Task<List<VersionMetadata>> ListVersionsAsync(string userId)
    {
        var prefix = $"users/{userId}/world-state/world/versions/";
    ...
    private class VersionedWorldState
    {
